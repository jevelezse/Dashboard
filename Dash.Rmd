---
title: " Variantes de una muestra poblacional colombiana"
output: 
...
fluidPage(visNetworkOutput("network"))
```
...
### Información demogrfica del grupo 1.
...
```{r echo=FALSE}
datos$EdadRango <- discretize(datos$Edad,method = "fixed", breaks  = c(-Inf,seq(0,60,10),Inf),labels = c("<0","0-9","10-19","20-29","30-39","40-49","50-59",">60"))
...
### Frecuencia de palabras en grupo 1
...
#rules <- apriori(genes, parameter = list(supp = 0.05, conf = 0.6, target = "rules",minlen=2))
...
```{r echo=FALSE}
knitr::include_graphics("cluster3.png")
```
...
#ules <- apriori(g, parameter = list(supp = 0.05, conf = 0.6, target = "rules",minlen=2))
...
```{r echo=FALSE}
knitr::include_graphics("cluster5.png")
```

Variantes en CFTR  y RB1
===================================== 


```{r}
#z <- selectInput("n_breaks", label = "Gene",
            #choices = c("RB1"), selected = 20)
```
...
#genes %<>% filter(Gene %in% c(z))#,"BRCA1","ATM","MLH2", "PMS1", "MSH2", "MSH6,
genes %<>% filter(Gene %in% c("CFTR"))#,"BRCA1","ATM","MLH2", "PMS1", "MSH2", "MSH6,
...
#rules_test <- rules[, by ="support"] # Filtro de reglas. 

rules_test <- rules[1:10, by ="confidence"] # Filtro de reglas. 
...
todo1<-visNetwork(nodes, edges, width = "90%") %>% 
  visOptions(highlightNearest = TRUE, nodesIdSelection = FALSE) %>%
  #visNodes(shape = "square") %>%                        # square for all nodes
  visEdges(arrows ="to") %>%                            # arrow "to" for all edges
  visGroups(groupname = "regla") %>%    # darkblue for group "A"
